...
pub fn current_exe() -> io::Result<PathBuf> {
    // QNX 6.5 has no /proc/self/exefile; libc `_cmdname` returns the executable
    // path from the AT_EXEFILE auxv entry and works on 6.5 and 7.x alike.
    unsafe extern "C" {
        fn _cmdname(name: *mut c_char) -> *mut c_char;
    }
    let p = unsafe { _cmdname(ptr::null_mut()) };
    if !p.is_null() {
    ...
    // Fallback for 7.x where /proc/self/exefile exists.
