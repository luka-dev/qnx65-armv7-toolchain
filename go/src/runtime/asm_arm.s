...
TEXT setg<>(SB),NOSPLIT|NOFRAME,$0-0
	...
	//
	// QNX uses the libc-call model, so asmcgocall switches g to g0 on every
	// libc/syscall even with cgo disabled; the TLS copy of g must therefore
	// always track the g register, or a signal arriving during a libc call
	// (e.g. SIGCHLD from a child exiting under os/exec) makes sigtramp's
	// load_g recover a stale g and crash ("bad g in signal handler").
#ifdef GOOS_openbsd
	B	runtime·save_g(SB)
#else
#ifdef GOOS_qnx
