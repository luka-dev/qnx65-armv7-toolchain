...
func (l *Loader) LoadSyms(arch *sys.Arch) {
	...
	// symSize is only a capacity hint; a shared library built from packages that
	// are (nearly) empty on this target — e.g. golang.org/x/sys/cpu on qnx/arm,
	// where all cpu-detection is build-tagged out — yields symSize 0, and
	// make([]objSym, 1, 0) would panic (len 1 > cap 0). Floor the capacity at 1.
	if symSize < 1 {
		symSize = 1
	}
