...
func LinkerDeps(s *modload.State, p *Package) ([]string, error) {
	...
	if what := externalLinkingReason(s, p); what != "" && cfg.BuildContext.Compiler != "gccgo" {
		if !cfg.BuildContext.CgoEnabled {
			// QNX/arm uses a cgo-less external linker (the internal linker can't
			// emit shared objects). buildmode=shared/-linkshared thus work without
			// cgo; don't force runtime/cgo when cgo is disabled on qnx.
			if !(cfg.Goos == "qnx") {
