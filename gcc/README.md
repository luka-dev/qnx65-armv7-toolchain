...
## Status — ✅ DONE (C + full C++11/14, runtime-validated)
...
- **17 × SIGBUS = unaligned access** — all pass rebuilt with `-mno-unaligned-access`.
- **5 × SIGSEGV = nested-function trampolines** — QNX non-executable *stack*.
- **6 × SIGABRT** — `-O2` optimizer corners (pass at `-O0`; inherent to GCC 4.9.4).
- **1 × `eeprof-1`** — needs the `-p`/mcount profiling runtime.
